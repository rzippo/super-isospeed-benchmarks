...
public static class CurveLowerBounds
{
    ...
    /// <summary>
    /// Lower bounds the curve with a <see cref="RateLatencyServiceCurve"/>.
    /// </summary>
    ...
    /// <summary>
    /// Lower bounds the given set of points with a <see cref="RateLatencyServiceCurve"/>.
    /// </summary>
    ...
    /// <summary>
    /// Lower bounds the curve with a <see cref="SigmaRhoArrivalCurve"/>.
    /// </summary>
    /// <param name="curve"></param>
    /// <returns></returns>
    /// <exception cref="ArgumentException"></exception>
    /// <exception cref="InvalidCastException"></exception>
    public static SigmaRhoArrivalCurve SigmaRhoLowerBound(this Curve curve)
    {
        if (curve is SigmaRhoArrivalCurve sr)
            return sr;
        if (curve.PseudoPeriodSlope.IsInfinite)
            throw new ArgumentException("Trying to lower bound a curve with infinite sustained rate");
        if (curve.FirstNonZeroTime > 0)
            return new SigmaRhoArrivalCurve(0, 0);
        ...
        {
            var fromOriginRate = CornerPoints()
                .Select(p => p.Value / p.Time)
                .Min();
            return new SigmaRhoArrivalCurve(0, fromOriginRate);
        }
