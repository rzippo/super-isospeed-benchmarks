...
public class ConstantJumpsStaircaseCurve : Curve
{
    ...
    /// <summary>
    /// Builds a right-continuous staircase curve with jumps at each step.
    /// </summary>
    /// <param name="transientParams">Must contain either 0 or n*2 parameters (n > 0), each > 0.</param>
    ...
    {
        if (transientParams.Count != 0 && transientParams.Count % 2 != 0)
        ...
        while (tParamIndex < transientParams.Count)
        {
            ...
            var point = new Point(prevTime, prevValue + jumpHeight);
